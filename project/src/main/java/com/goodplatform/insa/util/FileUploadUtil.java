...
public class FileUploadUtil {
	...
	public List<HashMap<String, Object>> fileUpload(Map<String, MultipartFile> files, String uploadPath, String uploadDtlPath,int file_count) throws Exception {
		...
		try {
			...
			server_nm = local.getHostAddress();
		} catch (UnknownHostException e1) {
			e1.printStackTrace();
		}
		server_nm = server_nm.replace(".","-");
		...
		while (itr.hasNext()) {
			...
			if(file.getSize() > 0){
				...
				String newName = getTimeStamp() +"_"+ server_nm + "_" + file_count;
...
//					System.out.println(filename);
	...
	//엑셀 파일 folder 폴더에 저장
	public static void makeReportExcel(Workbook workbook, String folder, String filename, HttpServletRequest request) throws Exception {

		String savepath = request.getSession().getServletContext().getRealPath("/")+"WEB-INF"+File.separator+"files"+File.separator+"excel"+File.separator+folder;
